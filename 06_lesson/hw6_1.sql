...
SET @max_cnt := (SELECT COUNT(*) AS cnt FROM messages WHERE to_user_id = @selected_user_id
...
(SELECT COUNT(*) AS cnt2 FROM messages WHERE to_user_id = @selected_user_id
GROUP BY from_user_id ORDER BY cnt2 DESC LIMIT 1) AS cnt_msgs
FROM users
